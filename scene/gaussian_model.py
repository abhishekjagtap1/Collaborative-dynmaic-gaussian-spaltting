...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, args):
        ...
        # Initialize the semantic feature (from friend's code)
        self._semantic_feature = torch.empty(0)  # Initialize as empty for now
    ...
    def capture(self):
        return (
            ...
            self._semantic_feature,  # Capture semantic feature
        )
    ...
    def restore(self, model_args, training_args):
        (self.active_sh_degree,
         ...
         opt_dict,
         self.spatial_lr_scale,
         self._semantic_feature) = model_args  # Restore semantic feature
    ...
    def get_semantic_feature(self):
        return self._semantic_feature  # Added property for accessing the semantic feature

    def rewrite_semantic_feature(self, x):
        self._semantic_feature = x  # Method to rewrite semantic feature
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, time_line: int, semantic_feature_size: int,
        ...
        """        semantic_feature_dim = 10  # Replace with the actual dimension for your semantic features
        self._semantic_feature = nn.Parameter(
            torch.zeros((fused_point_cloud.shape[0], semantic_feature_dim)).float().cuda())  # New Line
        """
        ...
        # Initialize semantic feature from point cloud (with optional speedup)
        if speedup:
            semantic_feature_size = int(semantic_feature_size / 4)  # Speed up for segmentation by reducing size
    ...
    def training_setup(self, training_args):
        ...
        # Add _semantic_feature to parameter list
        l = [
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
            elif param_group["name"] == "semantic_feature":  # New Line
                lr = self.xyz_scheduler_args(iteration)  # Assuming same scheduler as xyz
                param_group['lr'] = lr
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._semantic_feature.shape[1]):  # Assuming shape: (N, semantic_dim)
            l.append('semantic_{}'.format(i))  # New Line
    ...
    def save_ply(self, path):
        ...
        # Semantic feature handling (added as per the friend's code)
        semantic_feature = self._semantic_feature.detach().transpose(1, 2).flatten(
            start_dim=1).contiguous().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        # Concatenate all attributes, including the semantic features
    ...
    def load_ply(self, path):
        ...
        # Semantic feature integration from your friend's code
        ...
        self._semantic_feature = nn.Parameter(
            torch.tensor(semantic_feature, dtype=torch.float, device="cuda").transpose(1,
                                                                                       2).contiguous().requires_grad_(
                True))  # Adding semantic feature
    ...
    def prune_points(self, mask):
        ...
        self._semantic_feature = optimizable_tensors["semantic_feature"]  # Added semantic feature pruning
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                              new_rotation, new_deformation_table, new_semantic_feature=None):
        d = {
            "xyz": new_xyz,
        ...
        # Add semantic feature if it's provided
        if new_semantic_feature is not None:
        ...
        # Add semantic feature update if it's available
        if new_semantic_feature is not None:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # Add semantic feature to densification if it's available
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation,
                                   new_deformation_table, new_semantic_feature)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20,
        ...
        # Add semantic feature if it's available
        new_semantic_feature = None
        if hasattr(self, "_semantic_feature"):
            new_semantic_feature = self._semantic_feature[selected_pts_mask]

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                                   new_rotation, new_deformation_table, new_semantic_feature)

...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_root", default='/home/uchihadj/ECCV_workshop/4DGaussians/data/s3_based_benchmarking/Semantic_20_3_sec_benchmarking_copy/cam01', type=str)
    ...
    for path in tqdm(sorted(os.listdir(IMAGE_DIR))):
        ...
        for idx, m in enumerate(masks):
            ...
            plt.show()
            if idx + 1 == 7:
                mask_image = Image.fromarray((mask_np * 255).astype(np.uint8))  # Convert boolean mask to uint8
                save_path = os.path.join(OUTPUT_DIR, f"{name}_mask_{idx + 1}.png")
                mask_image.save(save_path)
                print(f"Mask {idx + 1} saved as {save_path}")
